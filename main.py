...
# --- Smartlead -> Clay relay ---------------------------------------------------
# Clay's webhook source rejects any request over ~100 KB with HTTP 413. A reply
# that carries an inline base64 image (a signature logo, a screenshot) pushes
# Smartlead's LEAD_CATEGORY_UPDATED payload past that, the row never lands in
# Clay, and the positive gets no Slack card and no microsite (Cupix, 24 Sep:
# 113 KB). Point the account-level Smartlead webhook here instead of straight
# at Clay: this strips inline images, caps very long strings, and forwards the
# same payload shape to the same Clay source, so nothing in Clay changes.

CLAY_POSITIVE_WEBHOOK_URL = os.environ.get("CLAY_POSITIVE_WEBHOOK_URL", "").strip()
RELAY_KEY = os.environ.get("RELAY_KEY", "").strip()
_RELAY_MAX_BYTES = 90_000          # stay under Clay's limit with headroom
_RELAY_MAX_STR = 20_000            # longest single string kept after cleaning

_INLINE_IMG = re.compile(r"<img[^>]*src=[\"']data:[^\"']*[\"'][^>]*>", re.I)
_DATA_URI = re.compile(r"data:[a-z]+/[a-z0-9.+-]+;base64,[A-Za-z0-9+/=\s]+", re.I)


def _relay_clean(obj):
    """Recursively drop inline images and cap long strings. Keys and structure unchanged."""
    if isinstance(obj, dict):
        return {k: _relay_clean(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [_relay_clean(v) for v in obj]
    if isinstance(obj, str):
        s = _DATA_URI.sub("", _INLINE_IMG.sub("", obj))
        return s if len(s) <= _RELAY_MAX_STR else s[:_RELAY_MAX_STR] + " [truncated]"
    return obj


def _relay_forward(payload: dict):
    raw = json.dumps(payload)
    before = len(raw.encode())
    cleaned = _relay_clean(payload)
    body = json.dumps(cleaned)
    if len(body.encode()) > _RELAY_MAX_BYTES:
        # last resort: shrink every long string further until it fits
        global _RELAY_MAX_STR
        for cap in (8_000, 3_000, 1_000):
            _saved, _RELAY_MAX_STR = _RELAY_MAX_STR, cap
            cleaned = _relay_clean(payload); body = json.dumps(cleaned)
            _RELAY_MAX_STR = _saved
            if len(body.encode()) <= _RELAY_MAX_BYTES:
                break
    who = (cleaned.get("to_email") or cleaned.get("sl_lead_email")
           or (cleaned.get("lead_data") or {}).get("email") or "?")
    for attempt in range(3):
        try:
            r = requests.post(CLAY_POSITIVE_WEBHOOK_URL, data=body,
                              headers={"Content-Type": "application/json"}, timeout=20)
            print(f"relay -> clay {r.status_code} {who} bytes {before}->{len(body.encode())}", flush=True)
            if r.status_code < 500 and r.status_code != 429:
                return
        except Exception as e:
            print(f"relay -> clay error {who}: {e}", flush=True)
        time.sleep(2 * (attempt + 1))


@app.post("/smartlead-to-clay")
async def smartlead_to_clay(request: Request):
    """Account-level Smartlead webhook target. Acks at once, forwards in the background."""
    if RELAY_KEY and request.query_params.get("key") != RELAY_KEY:
        return JSONResponse({"status": "rejected"}, status_code=401)
    if not CLAY_POSITIVE_WEBHOOK_URL:
        print("relay: CLAY_POSITIVE_WEBHOOK_URL not set", flush=True)
        return JSONResponse({"status": "not_configured"}, status_code=503)
    try:
        payload = await request.json()
    except Exception:
        return JSONResponse({"status": "bad_json"}, status_code=400)
    _threading.Thread(target=_relay_forward, args=(payload,), daemon=True).start()
